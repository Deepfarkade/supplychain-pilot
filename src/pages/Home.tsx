...
const Home = () => {
  ...
  const bottomItems = [
    { icon: FileText, label: 'Raise a ticket', path: '/support' },
    ...
    <div className={`transition-opacity duration-300 ${mounted ? 'opacity-100' : 'opacity-0'}`}>
      ...
      >
        <div className="flex h-[calc(100vh-120px)]">
              ...
              <div className="space-y-1">
                {bottomItems.map((item, index) => (
                  <button
                    ...
                    onClick={() => handleNavigation(item.path)}
                    disabled={isLoading}
                    className="w-full flex items-center gap-2 px-2 py-1.5 rounded-md text-xs transition-all duration-200 text-foreground hover:bg-accent hover:text-accent-foreground"
                  >
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className="w-full h-7 text-xs bg-destructive/10 text-destructive hover:bg-destructive hover:text-destructive-foreground justify-start gap-2 mt-2"
                  onClick={logout}
                >
          ...
          {/* Main Content - Static Height with No Scroll */}
          <div className="flex-1 px-6 py-4 overflow-hidden">
            <div className="h-full flex flex-col space-y-6">
              ...
              <div className="animate-fade-in grid grid-cols-1 md:grid-cols-3 gap-4 flex-1" style={{ animationDelay: '200ms' }}>
              ...
              <div className="animate-fade-in" style={{ animationDelay: '400ms' }}>
                <div className="grid grid-cols-4 gap-4">
                  {[
                    { label: 'Active Apps', value: allMicroservices.length.toString(), icon: Activity },
                    { label: 'Sessions', value: '2,431', icon: Users },
                    { label: 'Efficiency', value: '35%', icon: TrendingUp },
                    { label: 'Savings', value: '$1.2M', icon: BarChart3 }
                  ].map((stat, index) => (
